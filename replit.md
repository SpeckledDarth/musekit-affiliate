...
- **Semantic CSS variables**: All UI colors use CSS variable-based semantic classes (`bg-background`, `text-foreground`, `bg-muted`, `text-muted-foreground`, `bg-card`, `text-card-foreground`, `border-border`, `ring-ring`). Variables defined in `src/app/globals.css` (:root + .dark), mapped in `tailwind.config.ts`. No hardcoded hex colors in Tailwind config — primary/accent palette scales (50-900) use CSS variables. Chart colors use `var(--chart-primary)`, `var(--chart-secondary)`, `var(--chart-tertiary)`. Only intentional gray: code block in tools page. Tailwind config has try/catch import for `@musekit/design-system` tailwindPreset (future-ready).
